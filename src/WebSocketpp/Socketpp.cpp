...
Socketpp::~Socketpp() {
  ...
  try {
    ...
    m_ws->join();
    m_endpoint.close(metadata->m_hdl, websocketpp::close::status::normal, "",
                     ec);
    if (ec) {
...
[[nodiscard]] std::pair<int, std::string> Socketpp::ws_request(
    const std::string& message) {
  // Send the message
  websocketpp::lib::error_code ec;
  auto metadata = con_metadata;

  m_endpoint.send(metadata->m_hdl, message, websocketpp::frame::opcode::text,
                  ec);
  if (ec) {
...
void Socketpp::switch_to_ws() {
  ...
  connection_metadata::ptr metadata_ptr(
      new connection_metadata(1, con->get_handle(), uri));
