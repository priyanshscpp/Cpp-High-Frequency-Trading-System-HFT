...
// Place Order Menu
int Trader::handlePlaceOrder(
    const std::function<std::pair<int, std::string>(std::string, double, int)>&
        action) {
  try {
    std::string ins;
    double price;
...
int Trader::handleCancelOrder(
    const std::function<std::pair<int, std::string>(std::string)>& action) {
  try {
    std::string order_id;
...
int Trader::handleGetOrderBook(
    const std::function<std::pair<int, std::string>(std::string, int)>&
        action) {
  try {
    std::string inst;
    int depth;
...
int Trader::handleMarketData(
  ...
  try {
    ...
    if (opt == 1) {
      std::string channel;
      std::cout << "Enter channel to subscribe: (eg. announcements, "
                   "platform_state, book.ETH-PERPETUAL.100ms) ";
      std::cin >> channel;
      ...
      std::string channel;
      std::cout << "Enter channel to unsubscribe: (eg. announcements, "
                   "platform_state, user.lock) ";
      std::cin >> channel;
...
// Constructor
Trader::Trader() : m_api(std::make_unique<Api>()) {
  std::cout << "New Api Instance Created\n";
}

// Function responsible for placing order
std::pair<int, std::string> Trader::place_order(const std::string& inst,
...
std::pair<int, std::string> Trader::modify_order(const std::string& order_id,
                                                 double price, int quant) {
  std::ostringstream payload;
  payload << R"({
        	"jsonrpc": "2.0",
...
std::pair<int, std::string> Trader::get_orderbook(const std::string& inst,
                                                  int depth) {
  std::ostringstream payload;
  payload << R"({
        	"jsonrpc": "2.0",
...
std::pair<int, std::string> Trader::get_marketdata(const std::string& channel,
                                                   int type) {
  std::ostringstream payload;
  if (type == 1) {
    payload << R"({
...
void Trader::Run() {
  ...
  while (run) {
    ...
    switch (inp) {
      case 1: {  // Place order
        int status = handlePlaceOrder(
            [this](const std::string& inst, double price, int quant) {
      ...
      case 2: {  // Cancel order
        int status = handleCancelOrder([this](const std::string& order_id) {
      ...
      case 3: {  // Modify order
        int status = handleModifyOrder(
            [this](const std::string& order_id, double price, int quant) {
      ...
      case 4: {  // Fetch order book logic here
        int status =
            handleGetOrderBook([this](const std::string& inst, int depth) {
      ...
      case 5: {  // Fetch positions logic here
        int status = handleViewPosition(
            [this](const std::string& inst) { return view_position(inst); });
        if (status) {
      ...
      case 6: {  // Start WebSocket market data stream here
        int status =
            handleMarketData([this](const std::string& kind, int type) {
      ...
      case 7: {  // Get Open Orders
        int status = handleOpenOrders(
            [this](const std::string& kind) { return get_openorders(kind); });
        if (status) {
